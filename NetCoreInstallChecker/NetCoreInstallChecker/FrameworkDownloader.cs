...
{
    private static readonly NuGetVersion _firstWindowsDesktopOutsideRuntimeVersion = new("5.0.0");
    ...
    /// <param name="name">THe name of the framework package.</param>
    ...
    public async Task<string> GetDownloadUrlAsync(Architecture arch, Platform platform = Platform.Windows,
        Format format = Format.Executable, bool tryGetLatestPatch = true)
    {
        var version = Version;
        ...
        switch (FrameworkName)
        {
            case FrameworkName.App:
                return BaseUrl +
                       $"/{directory}/{version}/dotnet-runtime-{version}-{platformIdentifier}-{archIdentifier}.{extension}";
            case FrameworkName.Asp:
                return BaseUrl +
                       $"/{directory}/{version}/aspnetcore-runtime-{version}-{platformIdentifier}-{archIdentifier}.{extension}";
            case FrameworkName.WindowsDesktop:
                ...
                {
                    return BaseUrl +
                           $"/{directory}/{version}/windowsdesktop-runtime-{version}-{platformIdentifier}-{archIdentifier}.{extension}";
                }
                else
                {
                    return BaseUrl +
                           $"/{FrameworkName.App.ToDownloadDirectory()}/{version}/windowsdesktop-runtime-{version}-{platformIdentifier}-{archIdentifier}.{extension}";
                }
            default:
    ...
    private string GetLatestPatchUrl()
    {
        ...
        switch (FrameworkName)
        {
            ...
            case FrameworkName.WindowsDesktop:
                // The windows desktop runtime is part of the core runtime url prior to 5.0
                if (Version >= _firstWindowsDesktopOutsideRuntimeVersion)
                    return BaseUrl + $"/{directory}/{twoPartVersion}/latest.version";
